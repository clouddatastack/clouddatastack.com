.. image:: ../images/yuri.jpeg
   :class: profile-photo
   :alt: Yuri Chernushenko

Data Platforms and Migrations
=============================

Preparing you for the AI era, I help you build strong foundations and radically improve maturity of your data and MLOps infrastructure to give you a competitive edge and a maximum return on investment.

I specialize in building custom cloud-native, future-proofed data platforms for enterprises. My services include designing and implementing `Enterprise Data Platforms <services/01_enterprise_data_platforms.html>`_, building infrastructure for `AI workflows <services/02_ai_workflows.html>`_ and facilitating `Data Migrations <services/03_data_migrations.html>`_. 

You can reach out to `me <https://yuri.chernushenko.com/>`_ via email `y.chernushenko@gmail.com <mailto:y.chernushenko@gmail.com>`_ or `schedule a call <https://calendly.com/y-chernushenko>`_.

.. toctree::
   :hidden:
   :maxdepth: 1
   :caption: Services:

   services/01_enterprise_data_platforms
   services/02_ai_workflows
   services/03_data_migrations

.. toctree::
   :hidden:
   :maxdepth: 1
   :caption: Technology Stack:

   stack/01_extraction
   stack/02_streaming
   stack/03_storage
   stack/04_compute
   stack/05_transformation
   stack/06_orchestration
   stack/07_governance
   stack/08_ai_workflows

.. raw:: html

   <script>
   var links = document.querySelectorAll('a[href^="http"]');
   links.forEach(function(link) {
      // Only open in new tab if the link is external
      if (!link.href.startsWith(window.location.origin)) {
         link.setAttribute('target', '_blank');
      }
   });
   </script>